...
- All rule files in `.cursor/rules/`
- All command files in `.cursor/commands/`
...
1. Consult any relevant rules in `.cursor/rules/` and start by listing which rule files have been used to guide the implementation (e.g. `Rules used: [clean-architecture.mdc, domain-driven-design.mdc]`).

2. Follow TDD when it is possible. Always start new changes by writing new test cases (or changing existing tests).
   Remember to consult [Unit and Integration Tests](./.cursor/rules/unit-and-integration-tests.mdc) for details on how to write tests with pytest.
...
3. Always run `uv run pytest` (or `pytest` with the `uv`-managed virtual environment activated) to verify that all tests pass before committing your changes. Never use `python -m pytest`.
 ...
 See [Python Tooling Execution](./.cursor/rules/python-tooling-execution.mdc) for the full policy on running tools and managing dependencies with `uv`.
